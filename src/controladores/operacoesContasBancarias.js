...
const atualizarContaBancaria = (req, res) => {
    ...
    const encontrarEmail = contas.find((conta) => {
        return conta.usuario.email === email && conta.numero !== numeroDaConta.numero;
    });
...
const transferenciaBancaria = (req, res) => {
    ...
    if(numeroContaOrigem === numeroContaDestino) {
        return res.status(400).json({
            mensagem: 'Não é possível realizar transferências bancárias para a sua própria conta!'
        })
    }

    const senhaContaOrigem = contas.find((conta) => {
        return conta.usuario.senha === senha && conta.numero === Number(numero_conta_origem);
    });
